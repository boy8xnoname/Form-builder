...
class EP_Mondula_Form_Wizard_Shortcode {
	...
	public function __construct(EP_Mondula_Form_Wizard $parent, $token, EP_Mondula_Form_Wizard_Wizard_Service $wizard_service) {
		$this->_parent = $parent;
		$this->_token = $token;
		...
		ep_multi_step_form_block_init(array($this, 'handler'));
	...
	**/
	 ...
	 * are deleted from the ep-temp directory. This function is called when
	 ...
	 * Temporarily upload a file to wp-content/uploads/ep-temp directory.
	 ...
	 public function fw_upload_file() {
		...
		*/
		function wpse_change_upload_dir_temporarily($dirs) {
			$dirs['subdir'] = '/ep-temp';
			$dirs['path'] = $dirs['basedir'] . '/ep-temp';
			$dirs['url'] = $dirs['baseurl'] . '/ep-temp';
			return $dirs;
		}
	...
	private function generate_attachment_paths($files) {
		...
		for ($i = 0; $i < count($files); $i++) {
			if ($files[ $i ] != '') {
				$attachments[ $i ] = WP_CONTENT_DIR . '/uploads/ep-temp/' . sanitize_file_name($files[ $i ]);
			}
		}
		return $attachments;
	}
	...
	private function verifyCaptcha() {
		...
		$secret = EP_Mondula_Form_Wizard_Wizard::fw_get_option('recaptcha_secretkey' ,'fw_settings_captcha', '');
