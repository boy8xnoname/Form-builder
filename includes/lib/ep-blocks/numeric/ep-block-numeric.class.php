<?php

if (!defined('ABSPATH')) exit;

/**
 * Representation of a numeric input field.
 */
class EP_Mondula_Form_Wizard_Block_Numeric extends EP_Mondula_Form_Wizard_Block {

	private $_label;
	private $_placeholder;	
	private $_required;
	private $_minimum;
	private $_maximum;
	private $_tooltipcontent;

	protected static $type = "fw-numeric";

	/**
	 * Creates an Object of this Class.
	 * @param string $label The Label the Object is being created with.
	 * @param boolean $required If true, Input for this field is required.
	 * @param integer $minimum Lower Threshold of Numeric Input.
	 * @param integer $maximum Upper Threshold of Numeric Input.
	 */
	public function __construct ($label, $placeholder, $tooltipcontent, $required, $minimum, $maximum, $fullwidth) {
		$this->_label = $label;
		$this->_placeholder = $placeholder;
		$this->_required = $required;
		$this->_minimum = $minimum;
		$this->_maximum = $maximum;
		$this->_fullwidth = $fullwidth;
		$this->_tooltipcontent = $tooltipcontent;
	}

	public function render($ids) {
		$minimumDefined = strlen($this->_minimum) > 0;
		$maximumDefined = strlen($this->_maximum) > 0;
		$showFieldIcon = (EP_Mondula_Form_Wizard_Wizard::fw_get_option('showfieldicon', 'fw_settings_styling', 'on') === 'on') ? 'on' : 'off';
		// var_dump($this->_fullwidth );
	?>
		<div 
			class="fw-step-block fw-step-icon-<?php echo $showFieldIcon;?>  <?php if($this->_fullwidth == 'true') {echo 'block-fullWidth';} else { echo 'block-halfWidth'; }?>" 
			data-blockId="<?php echo $ids[0]; ?>" 
			data-type="fw-numeric" 
			data-required="<?php echo $this->_required; ?>" 
			<?php if ($minimumDefined) { echo 'data-min="' . $this->_minimum . '"'; } ?>
			<?php if ($maximumDefined) { echo 'data-max="' . $this->_maximum . '"'; } ?>
		>

			<div class="fw-input-container">
				<h3>
					<?php echo $this->_label ?>
					<?php if(!empty($this->_tooltipcontent)) { ?> 
						<span class="ep_tooltip" data-toggle="tooltip" data-placement="right" data-custom-class="ep-tooltip" title="<?php echo $this->_tooltipcontent;?>">
							<i class="fa fa-info-circle"></i>
						</span>
					<?php } ?>
				</h3>
				<input 
					type="text" 
					class="fw-text-input" 
					data-id="numeric"
					placeholder="<?php echo $this->_placeholder ?>"
					id="ep-numeric-<?php echo str_replace(' ', '-', strtolower($this->_label)); ?>"
					placeholder="<?php 
						if ($minimumDefined && $maximumDefined) { 
							echo $this->_minimum . ' - ' . $this->_maximum;
						} else if ($minimumDefined) {
							echo __("min. ", 'multi-step-form') . $this->_minimum;
						} else if ($maximumDefined) {
							echo __("max. ", 'multi-step-form') . $this->_maximum;
						} else {
							echo "";
						}
					?>"
					<?php if ($minimumDefined) { echo 'min="' . $this->_minimum . '"'; } ?>
					<?php if ($maximumDefined) { echo 'max="' . $this->_maximum . '"'; } ?>
				>
				<?php if( $showFieldIcon == 'on') { ?>
					<span class="fa fa-asterisk form-control-feedback" aria-hidden="true"></span>
				<?php } ?>
			</div>
			<div class="fw-clearfix"></div>
		</div>
	  <?php
	}

	public function as_aa() {
		return array(
			'type' => 'numeric',
			'label' => $this->_label,
			'placeholder' => $this->_placeholder,
			'required' => $this->_required,
			'minimum' => $this->_minimum,
			'maximum' => $this->_maximum,
			'fullwidth' => $this->_fullwidth,
			'tooltipcontent' => $this->_tooltipcontent
		);
	}

	public static function from_aa($aa , $current_version, $serialized_version) {
		$label = $aa['label'];
		$placeholder = $aa['placeholder'];
		$required = $aa['required'];
		$minimum = $aa['minimum'];
		$maximum = $aa['maximum'];
		$fullwidth = $aa['fullwidth'];
		$tooltipcontent = isset($aa['tooltipcontent']) ? $aa['tooltipcontent'] : array();

		if (!ctype_digit(ltrim($minimum, '-'))) {
			$minimum = '';
		}

		if (!ctype_digit(ltrim($maximum, '-'))) {
			$maximum = '';
		}

		if (strlen($minimum) > 0 && strlen($maximum) > 0) {
			if (intval($minimum) >= intval($maximum)) {
				$maximum = strval(intval($minimum) + 1);
			}
		}

		return new EP_Mondula_Form_Wizard_Block_Numeric($label, $placeholder, $tooltipcontent, $required, $minimum, $maximum, $fullwidth );
	}

	public static function addType($types) {

		$types['numeric'] = array(
			'class' => 'EP_Mondula_Form_Wizard_Block_Numeric',
			'title' => __('Numeric', 'multi-step-form'),
			'show_admin' => true,
		);

		return $types;
	}
}

add_filter('multi-step-form/block-types', 'EP_Mondula_Form_Wizard_Block_Numeric::addType', 5);
