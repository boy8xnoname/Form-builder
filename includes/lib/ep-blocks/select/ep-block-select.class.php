 ...
 * @author alex
 */
class EP_Mondula_Form_Wizard_Block_Select extends EP_Mondula_Form_Wizard_Block {
	 ...
	 */
	public function __construct ($elements, $tooltipcontent, $required, $label, $placeholder, $search) {
	...
	public function render($ids) {
		...
		<div class="fw-step-block" data-blockId="<?php echo $ids[0]; ?>" data-type="fw-select" data-required="<?php echo $this->_required; ?>">	
			...
			<select class="fw-select" 
				...
				id="ep-select-<?php echo str_replace(' ', '-', strtolower($this->_label)); ?>">
				<?php if($this->_placeholder) { ?> 
					<option><?php echo $this->_placeholder; ?></option>
				<?php } ?>
				<?php for ($i = 0; $i < $cnt; $i++) {
	...
	public static function from_aa($aa , $current_version, $serialized_version) {
		...
		return new EP_Mondula_Form_Wizard_Block_Select($elements, $tooltipcontent, $required, $label, $placeholder, $search);
	}
	...
	public static function addType($types) {
		...
		$types['select'] = array(
			'class' => 'EP_Mondula_Form_Wizard_Block_Select',
...
add_filter('multi-step-form/block-types', 'EP_Mondula_Form_Wizard_Block_Select::addType', 1);
