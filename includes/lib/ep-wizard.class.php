...
class EP_Mondula_Form_Wizard_Wizard {
	...
	public function render($wizard_id) {
		...
		$show_summary = EP_Mondula_Form_Wizard_Wizard::fw_get_option('showsummary' ,'fw_settings_email', 'on') === 'on';
		$use_captcha = EP_Mondula_Form_Wizard_Wizard::fw_get_option('recaptcha_enable' ,'fw_settings_captcha') === 'on';
		$captcha_key = EP_Mondula_Form_Wizard_Wizard::fw_get_option('recaptcha_sitekey' ,'fw_settings_captcha', '');
		$captcha_invisible = EP_Mondula_Form_Wizard_Wizard::fw_get_option('recaptcha_invisible' ,'fw_settings_captcha', 'on') === 'on';
	...
	public function send_mail($data, $attachments = array()) {
		$mailformat = EP_Mondula_Form_Wizard_Wizard::fw_get_option('mailformat' ,'fw_settings_email', 'html');
		...
		if ($mail_success) {
			if (isset($settings['usercopy'])) {
				...
				$oldCc = EP_Mondula_Form_Wizard_Wizard::fw_get_option('cc' ,'fw_settings_email', 'off');
	...
	public static function from_aa($aa, $current_version, $serialized_version) {
		$wizard = new EP_Mondula_Form_Wizard_Wizard();
		$wizard->set_settings($aa['settings']);
		$wizard->set_title($aa['title']);
		foreach ($aa['steps'] as $step) {
			$wizard->add_step(
				EP_Mondula_Form_Wizard_Wizard_Step::from_aa($step, $current_version, $serialized_version)
			);
		}
		return $wizard;
	}
