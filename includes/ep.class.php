...
class EP_Mondula_Form_Wizard {

	/**
	 * The single instance of EP_Mondula_Form_Wizard.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	public function __construct ($file = '', $version = '1.0.4') {
		...
		$this->_token = 'EP_Mondula_Form_Wizard';
		...
		// Setup cronjob
		add_action('ep_cron_upload_clean', array($this, 'cleanup_upload_dir'));
		if (!wp_next_scheduled('ep_cron_upload_clean')) {
			wp_schedule_event(time(), 'daily', 'ep_cron_upload_clean');
		}

		// Set up service
		$this->_wizard_service = new EP_Mondula_Form_Wizard_Wizard_Service(
			new EP_Mondula_Form_Wizard_Wizard_Repository('EP_Mondula_Form_Wizards'),
				$this->_version
		);
		...
		if (is_admin()) {
			$this->admin = new EP_Mondula_Form_Wizard_Admin(
		...
		// Setup shortcode
		$this->shortcode = new EP_Mondula_Form_Wizard_Shortcode($this, $this->_token, $this->_wizard_service);
	...
	public function enqueue_scripts() {
		// Vendor
		wp_register_style($this->_token . '-vendor', esc_url($this->assets_url) . 'styles/ep-vendor.min.css', array(), $this->_version);
		wp_register_script($this->_token . '-vendor', esc_url($this->assets_url) . 'scripts/ep-vendor' . $this->script_suffix . '.js', array('jquery'), $this->_version, true);

		// CSS
		wp_register_style($this->_token . '-frontend', esc_url($this->assets_url) . 'styles/ep-frontend.min.css', array(), $this->_version);

		// JavaScript
		$i18n = $this->get_translation();
		wp_register_script($this->_token . '-frontend', esc_url($this->assets_url) . 'scripts/ep-frontend' . $this->script_suffix . '.js', array('jquery', 'jquery-ui-datepicker'), $this->_version, true);
		$ajax = array(
			'i18n' => $i18n,
	...
	public function cleanup_upload_dir() {
		...
		$files = list_files(WP_CONTENT_DIR . '/uploads/ep-temp/', 1);
		
		if (is_array($files)) {
			foreach ($files as $file) {
				/* File is older than 4 hours: */
				if (time() - filemtime($file) > (60 * 60 * 4)) {
					wp_delete_file_from_directory($file, WP_CONTENT_DIR . '/uploads/ep-temp/');
				}
			}
		}
	}
	...
	/**
	 * Main EP_Mondula_Form_Wizard Instance
	 *
	 * Ensures only one instance of EP_Mondula_Form_Wizard is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @see EP_Mondula_Form_Wizard()
	 * @return Main EP_Mondula_Form_Wizard instance
	 */
	...
	public function uninstall() {
		$timestamp = wp_next_scheduled('ep_cron_upload_clean');
		wp_unschedule_event($timestamp, 'ep_cron_upload_clean');
	}
