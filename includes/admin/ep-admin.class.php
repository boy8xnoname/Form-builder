 ...
 * @author alex
 */
class EP_Mondula_Form_Wizard_Admin {
	...
	public function __construct(EP_Mondula_Form_Wizard_Wizard_Service $wizard_service, $token, $assets_url, $script_suffix, $version) {
	...
	private function init() {
			...
			add_action('wp_ajax_setupFormApplyNowFromDemoData',  array($this, 'setupFormApplyNowFromDemoData'));
			add_action('wp_ajax_nopriv_setupFormApplyNowFromDemoData', array($this, 'setupFormApplyNowFromDemoData'));

			// add_action('wp_ajax_setupFormApplyNowFromDemoData',  'setupFormApplyNowFromDemoData');
			// add_action('wp_ajax_nopriv_setupFormApplyNowFromDemoData', 'setupFormApplyNowFromDemoData');
	}

	public function setup_menu() {
			$all = add_menu_page('EP Multi Step Form', 'EP Multi Step Form', 'manage_options', 'ep-multistep-forms', array($this, 'menu'), 'dashicons-feedback', '35');
			add_submenu_page('ep-multistep-forms', 'EP Multi Step Form List', 'Forms', 'manage_options', 'ep-multistep-forms', array($this, 'menu'), 0);
			$add = add_submenu_page('ep-multistep-forms', 'EP Multi Step Form Edit', 'Add New', 'manage_options', 'ep-multistep-forms&edit', array($this, 'menu'), 1);
			do_action('multi-step-form/submenus');
			add_submenu_page('ep-multistep-forms', 'EP Multi Step Form Troubleshooting', 'Troubleshooting', 'manage_options', 'ep-multistep-forms-ts', array($this, 'troubleshooting'));
	...
	public static function get_translation() {
		return array(
			...
			'partIDTitle' => __('Section Custom ID', 'multi-step-form'),
			'partClassTitle' => __('Section Custom Class', 'multi-step-form'),
			...
			'labelplaceholder' => __('Placeholder', 'multi-step-form'),
			'labeltooltip' => __('Tooltip', 'multi-step-form'),
			'multifile' => __('Multiple Files', 'multi-step-form'),
			'dateformat' => __('Date Format', 'multi-step-form'),
			'required' => __('Required', 'multi-step-form'),
			'fullwidth' => __('Field Full Width', 'multi-step-form'),
			'radio' => array(
	...
	public function admin_js() {
		if (isset($_GET['edit'])) {
			...
			$cc = EP_Mondula_Form_Wizard_Wizard::fw_get_option('cc' ,'fw_settings_email', 'off');

			wp_register_script($this->_token . '-backend', esc_url($this->_assets_url) . 'scripts/ep-backend' . $this->_script_suffix . '.js', array('postbox', 'jquery-ui-dialog', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-tooltip', 'jquery'), $this->_version);
			$ajax = array(
				'i18n' => $i18n,
			...
			wp_register_style($this->_token . '-backend', esc_url($this->_assets_url) . 'styles/ep-backend' . $this->_script_suffix . '.css', array(), $this->_version);
	...
	public function troubleshooting() {
		...
		require 'partials/ep-troubleshooting.php';
	}
	...
	public function table() {
		$table = new EP_Mondula_Form_Wizard_List_Table($this->_wizard_service);
	...
	public function download_form_json() {
		if (current_user_can('editor') || current_user_can('administrator')) {
			if (isset($_GET['page']) && $_GET['page'] === 'ep-multistep-forms'
			&& isset($_GET['export'])) {
				/* Get JSON for form */
				$json = $this->prettify_json($this->_wizard_service->get_as_json($_GET['export']));
				$filename = sanitize_file_name('ep-' . $_GET['export'] . '_' . date('Y-m-d H:i:s'));
	...
	public function setupFormApplyNowFromDemoData() {
		global $wpdb;	
		$prefix = $wpdb->prefix;
